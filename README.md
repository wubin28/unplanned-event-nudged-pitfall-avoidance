...
| 起始时间（北京时间从新到旧） | 持续时长（小时） | 官方复盘报告链接 | 影响范围 | 事件简介 | 恢复过程 | 官方报告触发因素 | 改进措施 | 出错界面截屏markdown文件链接 |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
| 2023.11.21 17:48 | 3.5 | [Elevated Errors on API and ChatGPT](https://status.openai.com/incidents/n254wyd7nml7) | 对 OpenAI 的大部分请求失败。付费和免费 ChatGPT 的失败率最高为 75%。| 2023年11 月 21 日，太平洋时间 1:48 - 5:21 之间，对 OpenAI 的大部分请求失败。付费和免费 ChatGPT 的失败率最高为 75%。| 在多次尝试恢复任一故障副本以及在其他区域提供替代副本均未成功后，通过与我们的服务提供商合作，**将同一区域中的两个新副本投入使用**，该问题最终得到缓解。作为事件响应的一部分，我们已经采取了以下措施：暂停类似的维护活动，直至另行通知。调整配置设置以**减少连接频繁断开并重连**。添加了一个功能标志，使我们可以基于百分比的部署将流量从副本转移。我们已经测试将 100% 的查询定向到主实例，并验证了我们对主实例失败原因的假设。 | 这次中断是**由 OpenAI 的主 API 服务使用的 PostgresDB 服务的维护**引发的，导致**两个只读副本同时失败**。尽管只有该数据库的只读副本受到影响，但它们是承载流量的，导致依赖于它们的请求路径失败。主要 API 服务在整个 OpenAI 技术栈中**提供用户身份验证和服务到服务身份验证**。 | 此外，我们将实施以下更改，以防止未来发生类似事件：从根本上来说，OpenAI 需要确保维护窗口不会导致大范围的服务中断。为此，我们将：**建立维护前审查流程**来审查所有即将进行的维护活动，以确保在发生意外停机时制定快速恢复计划。确保在维护活动期间有足够的缓冲容量作为“热”备用。确保托管关键服务的区域有足够的缓冲能力。**数据库维护导致副本离线/不同步，从而导致副本滞后无限增加。制定（短期）策略/自动化，以从副本滞后和（长期）架构更改中快速恢复，以消除此因素**。检查我们的整个基础设施和服务堆栈，以确保我们添加更多冗余和故障安全机制，以提高弹性。我们知道，长时间的 API 中断会影响客户的产品和业务，而这种程度的中断尤其具有破坏性。虽然我们在这方面做得不够，但我们致力于防止未来发生此类事件并提高我们的服务可靠性。 |  |
